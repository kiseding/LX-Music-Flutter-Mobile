...
|------|------|
| **搜索 / 排行榜** | QQ（腾讯）· 酷我 · 网易云 |
| **播放** | 后台播放、锁屏控件、全屏封面/歌词、迷你播放栏、音质（默认 320k）、均衡器 |
...
- 酷狗 / 咪咕
- 内置第三方音源 API（如 Huibq、`lxmusicapi.onrender.com`）
- Workers 侧搜索、播放 URL、歌词（均由 App + 自定义源完成）
...
点播
  │
  ├─① 已启用的自定义源脚本（API_URL 等由脚本自身决定，App 不硬编码）
  │     └─ 成功 → 播放
  │
  └─② 内置平台源 tx / kw / wy（官方接口）
        └─ 成功 → 播放
