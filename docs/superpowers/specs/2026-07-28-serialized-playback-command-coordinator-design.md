...
Intent APIs name their authority explicitly. `recordExplicitPlayIntent` and `recordExplicitPauseIntent` advance the coordinator intent revision; `setDesiredPlayingPreservingIntent` changes desired state without advancing it. Public `play`, next, previous, default direct queue selection, and playlist replacement record explicit play intent. `playAfterLoad: false`, automatic completion/fallback, preload, recovery, quality reload, and current-item replacement use preserving intent and cannot clear an interruption or becoming-noisy denial.

The read-only `desiredPlayingIntent` reports whether desired play still has authority after explicit pause/stop/noisy and non-resumable denial, while deliberately ignoring temporary active interruption and preserving-owner blocks. `effectivePlaying` additionally applies those temporary blocks. Quality reload snapshots `desiredPlayingIntent`, not native `AudioPlayer.playing`, so a reload begun while output is interruption-blocked preserves resumable play intent without bypassing a denial.
...
The coordinator serially installs an accepted source and reconciles afterward. A source spanning a complete resumable interruption starts after installation when user intent remains play. A non-resumable cycle or becoming-noisy event records a denial at the current explicit-intent revision; the source remains paused until a newer explicit play advances that revision. Public navigation that semantically starts playback is such an explicit play, so it clears the older denial and starts its selected source. Automatic and paused source transitions preserve the revision and remain blocked. Explicit play during an older paused selection therefore transfers fresh play authority to that still-authoritative source request.
