...
Future<void> ensureBackgroundReadable(String path) async {
  ...
  await _fileProtectionChannel.invokeMethod<void>('ensureBackgroundReadable', {
    'path': path,
  });
}

const mediaUserAgent =
...
Map<String, String> mediaRequestHeaders(String url, String platform) {
  ...
  return {'User-Agent': mediaUserAgent, 'Referer': referer, 'Accept': '*/*'};
}
...
typedef PlaybackLeaseAcquirer =
    Future<PlaybackCacheLease?> Function({
      required String remoteUrl,
      required String platform,
      required String songId,
      required String quality,
    });
typedef PlaybackStreamValidator =
...
class PlaybackCacheService {
  ...
  static String extensionFromBytes(
    List<int> bytes, {
  ...
  Future<void> _initialize() async {
    ...
    _root = _normalizeAbsolute(directory.path);
    _resolvedRoot = _normalizeAbsolute(await directory.resolveSymbolicLinks());
  ...
  Future<void> _drainForDispose(
    List<_InflightOperation> cancelledOperations,
  ) async {
  ...
  Future<String?> getOrDownload({
    ...
    return _getOrDownloadPath(
      key: cacheKey(platform: platform, songId: songId, quality: quality),
      remoteUrl: remoteUrl,
      platform: platform,
      songId: songId,
      quality: quality,
  ...
  Future<_PathDecision> _getOrStartLocked({
    ...
    if (hit != null) {
      ...
      final validated = persisted == null
          ? null
          : await _validatedStableEntry(persisted);
      if (validated == null) {
        _index.remove(key);
  ...
  Future<String?> _validatedExistingFile(String path) async {
    ...
    try {
      ...
      final resolved = _normalizeAbsolute(
        await File(lexical).resolveSymbolicLinks(),
      );
      return _isResolvedRootChild(resolved) ? lexical : null;
    } on FileSystemException {
      return null;
    }
  }
  ...
  Future<String?> _validatedDestination(String path) async {
    ...
    try {
      ...
      if (!_isResolvedRootOrChild(resolvedParent)) return null;
  ...
  bool _isResolvedRootChild(String path) {
  ...
  bool _isResolvedRootOrChild(String path) =>
      path == _resolvedRoot || _isResolvedRootChild(path);
  ...
  Future<void> _enforceSizeCap() async {
    ...
    for (final candidate in ordered) {
      await _withKeyTransaction(candidate.key, () async {
        final total = _index.values.fold<int>(
          0,
  ...
  void _protectMalformedRecord(Object? item) {
    ...
    final lexical = _lexicalChild(path);
    if (lexical == null || File(lexical).parent.path != _root) return;
    final match = _stableCacheName.firstMatch(
      File(lexical).uri.pathSegments.last,
    );
  ...
  bool _looksLikeNonAudio(List<int> header) {
    ...
    final start = String.fromCharCodes(
      header.take(32),
