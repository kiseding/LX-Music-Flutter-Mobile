...
class CustomSourceService {
  ...
  Future<void> _loadSources() async {
    ...
    if (jsonStr != null) {
      ...
      final loaded = jsonList
          ...
          .map((source) {
            if (source.isEnabled && hasUnsafeSynchronousLoop(source.script)) {
              sanitized = true;
              return source.copyWith(isEnabled: false);
            }
            return source;
          })
          .toList();
      ...
      if (sanitized) await _saveSources(loaded);
    }
  }
  ...
  Future<T> _mutate<T>(
    ...
    final operation = _mutationTail.then((_) async {
      ...
      final sources = mutation.sources
          .map(
            (source) =>
                source.isEnabled && hasUnsafeSynchronousLoop(source.script)
  ...
  Future<bool> toggleSource(String id) async {
    ...
    if (willEnable && hasUnsafeSynchronousLoop(target.script)) return false;
  ...
  Future<bool> importLxMusicScript(String script) async {
    ...
    try {
      ...
      return await _mutate<bool>((current) {
        final now = _clock();
        final shouldEnable =
            !hasUnsafeSynchronousLoop(script) &&
  ...
  Future<bool> importSourceFromUrl(
    ...
    try {
      return await () async {
        final response = await _importSandbox.request(Uri.parse(url), const {
          'method': 'GET',
          'timeout': 15000,
        }, cancellation: cancel);
