...
class _DownloadScreenState extends ConsumerState<DownloadScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('下载管理', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w600, color: AppColors.onScaffold(context))),
          actions: [
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                PopupMenuItem(value: 'pause_all', child: Text('暂停全部', style: TextStyle(color: AppColors.onScaffold(context)))),
                PopupMenuItem(value: 'clear_completed', child: Text('清理已完成', style: TextStyle(color: AppColors.error))),
              ],
            ),
          ],
        ),
        body: tasks.isEmpty
            ? Center(
                child: Column(
                  ...
                  children: [
                    Icon(Icons.download_done, size: 64, color: AppColors.mutedText(context)),
                    const SizedBox(height: 16),
                    Text('暂无下载任务', style: TextStyle(color: AppColors.mutedText(context), fontSize: 16)),
                  ],
                ),
              )
            : Column(
                children: [
                  ...
                  Expanded(
                    child: filtered.isEmpty
                        ? Center(child: Text('暂无任务', style: TextStyle(color: AppColors.mutedText(context), fontSize: 14)))
                        : ListView.builder(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              Text('$completed / $total 已完成', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.onScaffold(context))),
              Text('${(progress * 100).toStringAsFixed(0)}%', style: TextStyle(fontSize: 11, color: AppColors.mutedText(context))),
            ],
          ),
          const SizedBox(height: 12),
          // 速度/时间/大小
                ...
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
                        Icon(Icons.pause, size: 14, color: AppColors.secondaryText(context)),
                        const SizedBox(width: 6),
                        Text('暂停全部', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.secondaryText(context))),
                      ],
                    ),
                  ),
                ),
              ...
              Expanded(
                child: GestureDetector(
                  ...
                  child: Container(
                    ...
                    alignment: Alignment.center,
                    child: Text('清理已完成', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.error)),
                  ),
                ),
              ),
            ],
  ...
  Widget _buildStatItem(String value, String label) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(value, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.onScaffold(context), fontFeatures: [FontFeature.tabularFigures()])),
        const SizedBox(height: 2),
        Text(label, style: TextStyle(fontSize: 10, color: AppColors.mutedText(context), letterSpacing: 0.06)),
      ],
    );
  }
          ...
          return Expanded(
            child: GestureDetector(
              ...
              child: Container(
                height: 34,
                decoration: BoxDecoration(
                  color: isActive ? AppColors.accentOf(context).withAlpha(51) : Colors.transparent,
                  border: isActive ? Border.all(color: AppColors.accentOf(context).withAlpha(77)) : null,
                  borderRadius: BorderRadius.circular(999),
                ),
                alignment: Alignment.center,
                child: Text(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      ...
                      color: isFailed ? AppColors.error : AppColors.onScaffold(context),
                    ),
                    maxLines: 1,
                  ...
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 11,
                      color: isFailed ? AppColors.error : AppColors.mutedText(context),
                    ),
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                  ),
  ...
  Widget _buildStatusWidget(BuildContext context, WidgetRef ref, DownloadTask task) {
    ...
    switch (task.status) {
      ...
      case DownloadStatus.paused:
        return IconButton(
          icon: Icon(Icons.play_circle_filled, color: AppColors.success, size: 20),
          onPressed: () => action('resume', task.id),
          padding: EdgeInsets.zero,
          constraints: const BoxConstraints(minWidth: 28, minHeight: 28),
        );
      case DownloadStatus.pending:
        return Icon(Icons.circle_outlined, color: AppColors.mutedText(context), size: 18);
      case DownloadStatus.failed:
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              border: Border.all(color: AppColors.accentOf(context).withAlpha(77)),
              borderRadius: BorderRadius.circular(3),
            ),
            child: Text('重试', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.accentOf(context))),
          ),
        );
    }
  }
  ...
  void _clearCompleted(BuildContext context, WidgetRef ref) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('清理已完成', style: TextStyle(color: AppColors.onScaffold(context))),
        content: Text('确定要清理 ${completed.length} 个已完成的下载记录吗？', style: TextStyle(color: AppColors.mutedText(context))),
        actions: [
