...
class LyricParser {
  ...
  static _ParsedLine? _parseLrcLine(String line) {
    ...
    if (millisecondsStr.isEmpty) {
      ...
      milliseconds =
          int.parse(millisecondsStr.substring(0, 3).padRight(3, '0'));
    }
  ...
  static List<LyricWord>? _parseWordTags(String body, {required Duration lineStart}) {
    // QRC 时钟格式
    ...
    final yrcLike = !body.trimLeft().startsWith('<') &&
        RegExp(r'^(?:[^<]+<-?\d+,-?\d+(?:,-?\d+)?>)+\s*[^<]*$').hasMatch(body.trim());
