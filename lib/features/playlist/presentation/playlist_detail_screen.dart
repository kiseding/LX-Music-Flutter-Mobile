...
class _PlaylistDetailScreenState extends ConsumerState<PlaylistDetailScreen> {
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            ...
            style:
                TextStyle(color: AppColors.onScaffold(context), fontSize: 18),
            maxLines: 1,
            ...
            if (!_isEditing && playlist.songs.isNotEmpty)
              IconButton(
                ...
                icon: Icon(Icons.play_circle_fill,
                    color: AppColors.accentOf(context), size: 28),
              ),
            if (_isEditing)
              TextButton(
                onPressed: () {
  ...
  Widget _buildNormalList(
      dynamic playerService, Playlist playlist, String? focusId) {
  ...
  void _showDeleteDialog(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Text('删除歌单',
            style: TextStyle(color: AppColors.onScaffold(context))),
        content: Text('确定删除「${playlist.name}」？',
