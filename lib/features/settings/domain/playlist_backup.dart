...
final class BackupData {
  const BackupData({
    ...
    this.autoResumePlayback = false,
    this.defaultSearchPlatform = 'tx',
  });
  ...
  final String defaultSearchPlatform;
}
...
BackupData decodeBackup(
  ...
  final audioQuality = _decodeAudioQuality(
    root['audio_quality'],
    'audio_quality',
  );
  final downloadQuality = _decodeAudioQuality(
    root['download_quality'],
    'download_quality',
  );
  final wifiOnlyDownload = _decodeWifiOnly(root['wifi_only_download']);
  final autoResumePlayback = _decodeBool(
    root['auto_resume_playback'],
    field: 'auto_resume_playback',
    fallback: false,
  );
  final defaultSearchPlatform = _decodeSearchPlatform(
    root['default_search_platform'],
  );
...
List<String> _decodeSearchHistory(Object? raw, {required BackupLimits limits}) {
  ...
  for (final item in raw) {
    if (item is! String || item.isEmpty) {
      throw const FormatException(
        'search_history items must be non-empty strings',
      );
    }
...
bool _decodeBool(Object? raw, {required String field, required bool fallback}) {
...
String _decodeSearchPlatform(Object? raw) {
  ...
  if (raw != 'tx' && raw != 'kw' && raw != 'wy') {
    throw const FormatException(
      'default_search_platform must be tx, kw, or wy',
    );
  }
...
final class BackupRestoreCoordinator {
  ...
  Future<void> restore(BackupData data) async {
    ...
    try {
      ...
      await _storage.setBool('auto_resume_playback', data.autoResumePlayback);
      await _storage.setString(
        'default_search_platform',
        data.defaultSearchPlatform,
      );
