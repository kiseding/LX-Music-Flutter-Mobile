...
class SettingsScreen extends ConsumerWidget {
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          title: Text('设置', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.onSurface)),
        ),
        body: ListView(
          children: [
            _buildSection(context, '外观', [
              _buildSwitchTile(context, ref, '深色模式', '使用深色主题', isDark && themeMode != ThemeMode.system, (value) {
                // 关闭「跟随系统」后，按开关设置深/浅色
                ref.read(themeModeProvider.notifier).setThemeMode(value ? ThemeMode.dark : ThemeMode.light);
              }),
              _buildSwitchTile(context, ref, '跟随系统', '自动切换亮色/深色主题', themeMode == ThemeMode.system, (value) {
                if (value) {
                  ref.read(themeModeProvider.notifier).setThemeMode(ThemeMode.system);
                } else {
                  // 退出跟随时，保持当前实际明暗
                  final darkNow = MediaQuery.platformBrightnessOf(context) == Brightness.dark;
                  ref.read(themeModeProvider.notifier).setThemeMode(darkNow ? ThemeMode.dark : ThemeMode.light);
                }
              }),
            ]),
            _buildSection(context, '语言', [
              _buildNavTile(context, ref, '语言', locale.languageCode == 'zh' ? '简体中文' : 'English', () => _showLanguageDialog(context, ref)),
            ]),
            _buildSection(context, '播放', [
              _buildNavTile(context, ref, '音质选择', _getQualityName(audioQuality), () => _showAudioQualityDialog(context, ref)),
              _buildNavTile(context, ref, '默认搜索平台', _platformName(ref.watch(defaultSearchPlatformProvider)), () => _showDefaultPlatformDialog(context, ref)),
              _buildNavTile(context, ref, '均衡器', ref.watch(equalizerProvider).preset.label, () => context.push('/equalizer')),
            ]),
            _buildSection(context, '下载', [
              _buildNavTile(context, ref, '下载管理', '查看和管理下载任务', () => context.push('/download')),
              _buildSwitchTile(context, ref, '仅 WiFi 下载', '仅在 WiFi 环境下下载歌曲', ref.watch(wifiOnlyDownloadProvider), (value) {
                ref.read(wifiOnlyDownloadProvider.notifier).setWifiOnly(value);
              }),
              _buildNavTile(context, ref, '下载音质', _getQualityName(ref.watch(downloadQualityProvider)), () => _showDownloadQualityDialog(context, ref)),
            ]),
            _buildSection(context, '高级功能', [
              _buildNavTile(context, ref, '自定义源', '管理自定义音乐源', () => context.push('/custom-source')),
            ]),
            _buildSection(context, '同步', [
              _buildNavTile(context, ref, '云端账号 / 歌单', 'Workers 登录、导入歌单', () => context.push('/sync')),
            ]),
            _buildSection(context, '数据', [
              _buildNavTile(context, ref, '备份数据', '导出歌单、设置等数据到文件', () => _backupData(context, ref)),
              _buildNavTile(context, ref, '恢复数据', '从备份文件恢复数据', () => _restoreData(context, ref)),
              _buildNavTile(context, ref, '清除缓存', '清除下载缓存和临时文件', () => _clearCache(context)),
            ]),
    ...
    return Column(
      ...
      children: [
        Padding(
          ...
          child: Text(title, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 1.2, color: AppColors.mutedText(context))),
        ),
        Container(
  ...
  Widget _buildSwitchTile(BuildContext context, WidgetRef ref, String title, String subtitle, bool value, ValueChanged<bool> onChanged) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
      decoration: BoxDecoration(border: Border(bottom: BorderSide(color: AppColors.cardBorder(context)))),
      child: Row(
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(title, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.onScaffold(context))),
                const SizedBox(height: 2),
                Text(subtitle, style: TextStyle(fontSize: 11, color: AppColors.mutedText(context))),
              ],
            ),
          ),
  ...
  Widget _buildNavTile(BuildContext context, WidgetRef ref, String title, String subtitle, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Icon(Icons.chevron_right, color: AppColors.mutedText(context), size: 18),
          ],
        ),
      ),
    );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: value ? Border.all(color: Colors.transparent) : Border.all(color: AppColors.cardBorder(context)),
        ),
        child: AnimatedAlign(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: value ? Theme.of(context).colorScheme.onPrimary : AppColors.onScaffold(context),
  ...
  String _getQualityName(AudioQualityOption quality) {
    switch (quality) {
      ...
      case AudioQualityOption.lossless24: return '臻品母带 (FLAC 24bit)';
      case AudioQualityOption.hires: return 'Hi-Res';
    }
  }

  String _platformName(String id) {
    switch (id) {
      case 'tx': return '腾讯 (QQ 音乐)';
      case 'kw': return '酷我';
      case 'wy': return '网易云';
      default: return id;
    }
  ...
  void _showDefaultPlatformDialog(BuildContext context, WidgetRef ref) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('默认搜索平台', style: TextStyle(color: AppColors.onScaffold(context))),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: ['tx', 'kw', 'wy'].map((id) {
            final current = ref.watch(defaultSearchPlatformProvider);
            return ListTile(
              title: Text(_platformName(id), style: TextStyle(color: current == id ? AppColors.accentOf(context) : AppColors.onScaffold(context))),
              trailing: current == id ? Icon(Icons.check, color: AppColors.accentOf(context)) : null,
              onTap: () {
                ref.read(defaultSearchPlatformProvider.notifier).setPlatform(id);
  ...
  void _showLanguageDialog(BuildContext context, WidgetRef ref) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.dialogBg(context),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('选择语言', style: TextStyle(color: AppColors.onScaffold(context))),
  ...
  Widget _buildLanguageOption(BuildContext context, WidgetRef ref, String name, Locale locale) {
    ...
    return ListTile(
      title: Text(name, style: TextStyle(color: currentLocale == locale ? AppColors.accentOf(context) : AppColors.onScaffold(context))),
      trailing: currentLocale == locale ? Icon(Icons.check, color: AppColors.accentOf(context)) : null,
  ...
  void _showQualityDialog(BuildContext context, WidgetRef ref, String title, bool isDownload) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.dialogBg(context),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text(title, style: TextStyle(color: AppColors.onScaffold(context))),
        content: Column(mainAxisSize: MainAxisSize.min, children: AudioQualityOption.values.map((quality) {
          ...
          return ListTile(
            title: Text(_getQualityName(quality), style: TextStyle(color: currentQuality == quality ? AppColors.accentOf(context) : AppColors.onScaffold(context))),
            trailing: currentQuality == quality ? Icon(Icons.check, color: AppColors.accentOf(context)) : null,
            onTap: () {
              if (isDownload) {
  ...
  Future<void> _clearCache(BuildContext context) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.dialogBg(context),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('清除缓存', style: TextStyle(color: AppColors.onScaffold(context))),
        content: Text('确定要清除所有缓存数据吗？下载的文件不会被删除。', style: TextStyle(color: AppColors.secondaryText(context))),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: Text('取消', style: TextStyle(color: AppColors.mutedText(context)))),
          TextButton(onPressed: () => Navigator.pop(context, true), child: Text('确定', style: TextStyle(color: AppColors.accentOf(context)))),
        ],
      ),
    );
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(child: Text(name, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: AppColors.onScaffold(context)))),
          Text(value, style: TextStyle(fontSize: 12, color: AppColors.mutedText(context))),
        ],
      ),
    );
  }
