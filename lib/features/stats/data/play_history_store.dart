...
class PlayHistoryStore extends ChangeNotifier {
  PlayHistoryStore(this._storageLoader, {DateTime Function()? clock})
  ...
  void tick(Duration position) {
    ...
    final previousTickAt = _lastTickAt;
    _lastPosition = position;
    _lastTickAt = now;
    if (previousPosition == null || previousTickAt == null) return;

    final positionDelta = position - previousPosition;
    final wallDelta = now.difference(previousTickAt);
    if (positionDelta <= Duration.zero || wallDelta <= Duration.zero) return;
    if (positionDelta > wallDelta + const Duration(seconds: 2)) return;
    _listenedSec +=
        (positionDelta < wallDelta ? positionDelta : wallDelta).inMilliseconds /
            1000.0;
  }
  ...
  void endSession() {
    ...
    final listenedSec = _listenedSec;
    _currentSongId = null;
    _startedAt = null;
    _listenedSec = 0;
    _lastPosition = null;
  ...
  ({int totalPlays, double totalSec, int activeDays, int uniqueSongs}) summary(
    ...
    final days = scoped
        .map(
          (e) => DateTime(e.playedAt.year, e.playedAt.month, e.playedAt.day),
        )
        .toSet();
    return (
