...
MusicItem decodeCloudSong(Object? raw) {
  ...
  if (songmid.isEmpty && hash.isEmpty) {
    throw FormatException(
      'cloud song requires non-empty songmid or hash',
      raw,
    );
  }
...
Future<CloudPlaylistMergeResult> mergeAndPersistCloudPlaylists({
  ...
  for (var i = 0; i < userList.length; i++) {
    ...
    if (name.isEmpty) {
      throw FormatException(
        'user playlist at [$i] name is required',
        raw,
      );
    }
    final list = raw['list'];
    if (list is! List) {
      throw FormatException(
        'user playlist at [$i] list must be a List',
        raw,
      );
    }
  ...
  if (favorites != null) {
    playlists['favorites'] = favorites.copyWith(
      songs: List<MusicItem>.unmodifiable(favoriteSongs),
      updatedAt: now(),
    );
  }
