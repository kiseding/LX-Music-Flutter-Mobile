...
void main() {
  ...
  test('stream validation requires a ranged audio response', () async {
    ...
    expect(await validate((_) => ResponseBody.fromBytes(mp3, 403)), isFalse);
    expect(await validate((_) => ResponseBody.fromBytes(mp3, 200)), isFalse);
    expect(
      await validate(
  ...
  test('protects cache directory and stable media before leasing', () async {
    ...
    cache = PlaybackCacheService(
      ...
      downloader: (url, savePath, {cancelToken}) async {
        await File(
          savePath,
        ).writeAsBytes([0x66, 0x4c, 0x61, 0x43, ...List<int>.filled(28, 0)]);
      },
    );
  ...
  test(
    'keeps lexical cache paths when the root resolves through a link',
    () async {
      if (Platform.isWindows) return;
      final realRoot = await Directory.systemTemp.createTemp('cache_real_');
      final linkedRoot = Link('${tempDir.parent.path}/cache_link_root');
      addTearDown(() async {
        if (await linkedRoot.exists()) await linkedRoot.delete();
        if (await realRoot.exists()) await realRoot.delete(recursive: true);
      });
      try {
        await linkedRoot.create(realRoot.path);
      } on FileSystemException {
        return;
      }
      await cache.dispose();
      cache = PlaybackCacheService(
        cacheRootOverride: linkedRoot.path,
        indexStore: MemoryPlaybackCacheIndexStore(),
        downloader: (url, savePath, {cancelToken}) async {
          await File(
            savePath,
          ).writeAsBytes([0x49, 0x44, 0x33, ...List<int>.filled(29, 0)]);
        },
      );
      ...
      expect(path, startsWith('${linkedRoot.path}${Platform.pathSeparator}'));
  ...
  test(
    'flac quality rejects content without a native FLAC signature',
    () async {
      final path = await cache.getOrDownload(
  ...
  test(
    'leased entry survives expiration until its idempotent release',
    () async {
  ...
  test(
    'acquireExisting persists last access before leasing a valid cache file',
    () async {
      var now = DateTime(2026, 1, 1);
  ...
  test(
    'acquireExisting rejects the lease when access persistence fails',
    () async {
  ...
  test(
    'cache hits persist lastAccessedAt and size eviction uses true LRU',
    () async {
      var now = DateTime(2026, 1, 1);
      final store = MemoryPlaybackCacheIndexStore();
      final lruCache = PlaybackCacheService(
  ...
  test(
    'rejects poisoned outside-root index without deleting outside file',
    () async {
  ...
  test(
    'rejects a persisted symlink escape without deleting its target',
    () async {
      if (Platform.isWindows) return;
  ...
  test(
    'cancelKey is a no-op after commit and reacquire keeps its lease',
    () async {
      final key = PlaybackCacheService.cacheKey(
        platform: 'tx',
        songId: 'idle-cancel',
  ...
  test(
    'repeated cancel targets the current replacement, not stale work',
    () async {
      final firstStarted = Completer<void>();
      final finishFirst = Completer<void>();
  ...
  test(
    'late cancelled downloader cannot detach newer inflight cancellation',
    () async {
      final firstStarted = Completer<void>();
      final finishFirst = Completer<void>();
      final secondStarted = Completer<void>();
  ...
  test(
    'serialized index writes cannot persist an older snapshot last',
    () async {
  ...
  test(
    'dispose waits for token-ignoring inflight cleanup and rejects new work',
    () async {
      final downloadStarted = Completer<void>();
      ...
      expect(
        tempDir.listSync().whereType<File>().where(
          (file) => file.path.endsWith('.part') || file.path.contains('.stage'),
        ),
        isEmpty,
      );
  ...
  test(
    'failed durable commit rolls back owned file and later write recovers',
    () async {
      final store = _ControlledIndexStore();
  ...
  test(
    'failed cache-hit persistence restores metadata and queue recovers',
    () async {
      var now = DateTime(2026, 1, 1);
      final store = _ControlledIndexStore();
      final durableCache = PlaybackCacheService(
  ...
  test(
    'overlapping hit failure cannot roll back newer access metadata',
    () async {
      var now = DateTime(2026, 1, 1);
      final store = _ControlledIndexStore();
  ...
  test(
    'format transition leaves physical stable bytes within maxBytes',
    () async {
      var now = DateTime(2026, 1, 1);
      final sizeCache = PlaybackCacheService(
        cacheRootOverride: tempDir.path,
      ...
      final stableFiles = tempDir.listSync().whereType<File>().where(
        (file) => RegExp(
          r'/[0-9a-f]{40}\.(mp3|flac|m4a|aac|ogg|wav|ape)$',
        ).hasMatch(file.path),
      );
      final physicalBytes = stableFiles.fold<int>(
        0,
  ...
  test('init removes unindexed known stable cache files only', () async {
    ...
    final store = MemoryPlaybackCacheIndexStore()
      ..value = jsonEncode([_entryJson(key: indexedKey, path: indexed.path)]);
  ...
  test(
    'mismatched persisted key path is dropped without deleting other key',
    () async {
      ...
      final store = MemoryPlaybackCacheIndexStore()
        ..value = jsonEncode([_entryJson(key: keyA, path: fileB.path)]);
  ...
  test(
    'persisted stable path must be a direct child with exact basename',
    () async {
      ...
      final store = MemoryPlaybackCacheIndexStore()
        ..value = jsonEncode([_entryJson(key: key, path: nestedFile.path)]);
      final strictCache = _cacheForPersistedPath(tempDir, store);
      await cache.dispose();
      cache = strictCache;
  ...
  test('persisted key must be lowercase forty hex characters', () async {
    ...
    final file = File('${tempDir.path}/$invalidKey.mp3')
      ..writeAsBytesSync(List<int>.filled(32, 4));
    final store = MemoryPlaybackCacheIndexStore()
      ..value = jsonEncode([_entryJson(key: invalidKey, path: file.path)]);
    final strictCache = _cacheForPersistedPath(tempDir, store);
    await cache.dispose();
    cache = strictCache;
  ...
  test(
    'exact outside-root symlink is removed without deleting target',
    () async {
      if (Platform.isWindows) return;
      ...
      final store = MemoryPlaybackCacheIndexStore()
        ..value = jsonEncode([_entryJson(key: key, path: link.path)]);
      final strictCache = _cacheForPersistedPath(tempDir, store);
      await cache.dispose();
      cache = strictCache;
  ...
  test('commit rejects a hostile stable destination symlink', () async {
    ...
    final hostileCache = PlaybackCacheService(
      ...
      downloader: (url, savePath, {cancelToken}) async {
        await File(
          savePath,
        ).writeAsBytes([0x49, 0x44, 0x33, ...List<int>.filled(29, 1)]);
        await link.create(target.path);
      },
    );
    await cache.dispose();
  ...
  test(
    'load ignores huge persisted size and keeps physically small entry',
    () async {
      ...
      final store = MemoryPlaybackCacheIndexStore()
        ..value = jsonEncode([
          {..._entryJson(key: key, path: file.path), 'sizeBytes': 999999},
        ]);
  ...
  test(
    'malformed top-level index preserves persisted data and stable files',
    () async {
  ...
  test(
    'mixed valid and malformed records repair without deleting claims',
    () async {
  ...
  test(
    'lease acquisition losing to ttl purge retries to a valid file',
    () async {
      var now = DateTime(2026, 1, 1);
  ...
  test(
    'dispose drains gated acquireOrDownload and prevents a late lease',
    () async {
      final validationStarted = Completer<void>();
      final releaseValidation = Completer<void>();
  ...
  test(
    'lease acquisition losing to size purge retries to a valid file',
    () async {
      var now = DateTime(2026, 1, 1);
      final validationStarted = Completer<void>();
      final releaseValidation = Completer<void>();
  ...
  test(
    'commit serializes before purge and preserves committed generation',
    () async {
      final store = _ControlledIndexStore();
      var writesAtStart = 0;
      final gatedCache = PlaybackCacheService(
