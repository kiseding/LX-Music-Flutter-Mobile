...
void main() {
  ...
  group('PlaybackUrlResolver', () {
    test('denied QQ HTTP media never reaches the native player', () async {
      ...
      final resolver = PlaybackUrlResolver<MusicItem>(
        resolvePlayableUrl: (music, {required preferredQuality}) async =>
            _playResult(
              url: 'http://wx.music.tc.qq.com/M800denied.mp3',
              platform: 'tx',
            ),
        acquireOrDownload: ({
          required remoteUrl,
    ...
    test('validated HTTP media can stream when cache transport is blocked',
      ...
      final resolver = PlaybackUrlResolver<MusicItem>(
        resolvePlayableUrl: (music, {required preferredQuality}) async =>
            _playResult(
              url: 'http://nf.sycdn.kuwo.cn/a.mp3',
              platform: 'kw',
            ),
        acquireOrDownload: ({
          required remoteUrl,
      ...
      final result = await resolver.resolve(_item(platform: 'kw'),
          preferredQuality: '128k');

      expect(result, isA<StreamingPlayback>());
      expect(result?.playableUrl, 'http://nf.sycdn.kuwo.cn/a.mp3');
    });
    ...
    test('source-resolved quality downgrade is cached once', () async {
      ...
      final result = await resolver.resolve(_item(), preferredQuality: 'flac');
    ...
    test(
      'range-validated lossy URL streams when local caching fails',
        () async {
    ...
    test(
      'lossless URL never streams even when range validation would pass',
        () async {
      var validationCalls = 0;
      final resolver = PlaybackUrlResolver<MusicItem>(
        resolvePlayableUrl: (music, {required preferredQuality}) async =>
  ...
  group('PlaybackLeaseSession', () {
    test(
      'old lease retained until newer source commit, then released once',
        () async {
    ...
    test(
      'releaseAll continues from pending failure to active release',
        () async {
  ...
  group('LxAudioHandler cached file reuse', () {
    test(
      'preloaded cached file is re-leased before authoritative reuse',
        () async {
    ...
    test(
      'old active lease releases only after replacement source commits',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'rejected cache candidate is not installed and re-resolves once',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test('ordinary local file installs without a cache lease', () async {
      ...
      await handler.setPlaylist([
        _cachedItem('local-boundary', '/tmp/local.mp3'),
      ]);
    ...
    test(
      'cached classification follows an occurrence moved while awaiting',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'resolver file classification rejects a replaced occurrence lease',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      ...
      final staleUrl = PlaybackCacheService.toPlayableUri(stale.path);
        final replacementUrl = PlaybackCacheService.toPlayableUri(
          '/cache/new.mp3',
        );
    ...
    test(
      'failed cached source install releases the newly acquired lease',
        () async {
      final player = _ReuseAudioPlayer()
    ...
    test(
      'late cached resolution after stop releases without becoming pending',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'streaming resolution discards an earlier pending cached lease',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'source switch rejects and releases the prior pending resolution',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'late cached A resolution after B switch cannot publish A extras',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'current resolution atomically publishes extras and stages its lease',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'skip to the second duplicate installs its exact occurrence',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'stale install recovery reloads the second duplicate occurrence',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'active occurrence moved during resolution still installs once',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'replacing an occurrence during resolution rejects its result',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
    ...
    test(
      'preload resolution patches the requested second duplicate occurrence',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
      ...
      handler.urlResolver = (id, [extras]) async {
        ...
        final resolution = const StreamingPlayback(
          'https://cdn.example/second.mp3',
            {'url': 'https://cdn.example/second.mp3', 'actualQuality': 'flac'},
        );
        expect(
    ...
    test(
      'overlapping preload schedules share an occurrence and allow retry',
      () async {
        final player = _ReuseAudioPlayer();
        final handler = LxAudioHandler(player: player);
        addTearDown(player.dispose);
        ...
        handler.urlResolver = (id, [extras]) async {
          ...
          return 'https://cdn.example/retry.mp3';
        };

        await handler.setPlaylist([
          _cachedItem('current', '/tmp/current.mp3'),
          _unresolvedItem('next'),
        ]);
        await firstStarted.future;
        expect(handler.debugPreloadRequestCount, 1);

        handler.debugSchedulePreload();
        await pumpEventQueue();
        expect(preloadCalls, 1);

        releaseFirst.complete();
        await pumpEventQueue();
        expect(handler.debugPreloadRequestCount, 0);

        handler.debugSchedulePreload();
        await pumpEventQueue();
        expect(preloadCalls, 2);
        expect(
          handler.queueItems[1].extras?['url'],
          'https://cdn.example/retry.mp3',
        );
        expect(handler.debugPreloadRequestCount, 0);
      },
    );
    ...
    test(
      'failed preload removes request authority and permits retry',
      () async {
        final player = _ReuseAudioPlayer();
        final handler = LxAudioHandler(player: player);
        addTearDown(player.dispose);
        ...
        handler.urlResolver = (id, [extras]) async {
          ...
          preloadCalls++;
          if (preloadCalls == 1) throw StateError('preload failed');
    ...
    test(
      'late preload cannot overwrite an item after it becomes current',
        () async {
      final player = _ReuseAudioPlayer();
      final handler = LxAudioHandler(player: player);
      addTearDown(player.dispose);
...
MediaItem _unresolvedItem(String id) =>
    MediaItem(id: id, title: id, extras: const {'requestedQuality': '320k'});
