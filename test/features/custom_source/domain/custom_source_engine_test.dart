...
void main() {
  test('LX bridge exposes the mobile event contract', () {
    final bridge = File(
      'lib/features/custom_source/domain/custom_source_engine.dart',
    ).readAsStringSync();

    expect(bridge, contains("env: 'mobile'"));
    expect(bridge, contains('group: function() {}'));
    expect(bridge, contains('groupEnd: function() {}'));

    expect(bridge, contains('globalThis._requestHandler = handler;'));
    expect(bridge, isNot(contains('Function.prototype.bind = function()')));
    expect(bridge, isNot(contains('BIND-FIX')));
    ...
    expect(
      bridge,
      contains("setHeaderIfMissing('Accept', 'application/json')"),
    );
    ...
    expect(bridge, contains('var handler = globalThis._requestHandler;'));
    expect(bridge, contains("typeof handler !== 'function'"));
    expect(bridge, contains("scriptInfo['rawScript'] = source.script;"));

    // 能力拒绝是正常回退路径（musicUrl/lyric/pic），不应当作错误冒泡到 UI，
    // 否则源未声明 lyric 时会刷出"Source does not support lyric"红字。
    expect(bridge, isNot(contains("_emitError('Source does not support")));
  });

  test(
    'secureRandomBytes returns independent bytes with the requested shape',
    () {
  ...
  test('song list detail request includes the selected platform', () {
    expect(buildSongListDetailParams('list-id', 'wy', 3), {
      'action': 'songListDetail',
      'source': 'wy',
      'info': {'id': 'list-id', 'page': 3},
    });
  });

  test('LX randomBytes uses the secure Dart bridge and returns Uint8Array', () {
    final bridge = File(
      'lib/features/custom_source/domain/custom_source_engine.dart',
    ).readAsStringSync();
    ...
    expect(randomBytesBlock, contains('__lxSecureRandomBytes'));
    expect(bridge, contains("method == 'randomBytes'"));
    expect(bridge, contains("onMessage('lx_secure_random'"));
    expect(bridge, contains('__lxResetSecureRandomPool'));
    expect(bridge, isNot(contains('arr[i] = Math.floor(Math.random() * 256)')));
  });
  ...
  test('source runtime polyfill keeps official-style web primitives', () {
    ...
    expect(polyfill, contains('var binary=true'));
    expect(polyfill, contains('if(binary&&body)'));
