...
void main() {
  test(
    'paste classification uses the LX header and leaves JSON structured',
    () {
      expect(
        isValidSourceScript(
      ...
      expect(isValidSourceScript('{"id":"source","script":"..."}'), isFalse);
    },
  );
  testWidgets(
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            ...
            importCustomSourceFromUrlProvider.overrideWithValue(
              (_) => completion.future,
            ),
          ],
  ...
  testWidgets('closing log dialog cancels its event subscription', (
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          customSourceServiceProvider.overrideWithValue(_NoopService()),
          customSourceEventStreamProvider.overrideWith(
            (ref, sourceId) => stream,
          ),
        ],
        child: const MaterialApp(home: CustomSourceScreen()),
