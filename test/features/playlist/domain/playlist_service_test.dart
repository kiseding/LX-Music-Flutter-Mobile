...
MusicItem detailedSong(String id, {String? name, Map<String, dynamic>? meta}) =>
    MusicItem(
      id: id,
      name: name ?? 'Song $id',
      singer: 'Artist $id',
...
Playlist playlist(String id, {String? name, List<MusicItem> songs = const []}) {
  return Playlist(
    id: id,
...
void main() {
  test(
    'successful mutation writes before publishing exactly one revision',
    () async {
  ...
  test(
    'init restores missing system playlists with one durable revision',
    () async {
      final repository = ControlledPlaylistRepository(
  ...
  test('recent is durably capped at 100 songs in newest-first order', () async {
    ...
    final repository = MemoryPlaylistRepository(
      PlaylistSnapshot(
        schemaVersion: 1,
        playlists: [playlist('favorites'), playlist('recent', songs: oldSongs)],
      ),
    );
  ...
  test(
    'replaceAll repairs system lists before one save and revision',
    () async {
      final repository = ControlledPlaylistRepository(
        initial: systemSnapshot(),
      );
  ...
  test(
    'restoreSnapshot always saves then publishes even if equivalent',
    () async {
  ...
  test(
    'init rejects empty and duplicate loaded ids without publication',
    () async {
      for (final invalid in [
        ...
        final repository = MemoryPlaylistRepository(
          PlaylistSnapshot(schemaVersion: 1, playlists: invalid),
        );
        final service = PlaylistService(repository: repository);
  ...
  test(
    'create rejects explicit, generated, and protected id collisions',
    () async {
      final repository = MemoryPlaylistRepository(
        systemSnapshot(additional: [playlist('custom')]),
      );
  ...
  test(
    'updatePlaylist persists same-id song content and nested meta changes',
    () async {
      final original = detailedSong(
        'a',
        meta: {
          'quality': {'bitrate': 128},
        },
      );
      final refreshed = detailedSong(
        'a',
        meta: {
          'quality': {'bitrate': 320},
        },
  ...
  test('addSongToPlaylist persists the timestamped replacement', () async {
    ...
    final savedPlaylist = repository.saves.single.playlists.firstWhere(
      (playlist) => playlist.id == 'custom',
    );
  ...
  test(
    'mutations invoked after disposal starts are rejected without save',
    () async {
      final repository = MemoryPlaylistRepository(systemSnapshot());
      final service = PlaylistService(repository: repository);
      await service.init();
  ...
  test(
    'concurrent init coalesces repair and retries after load failure',
    () async {
      final repository = RetryLoadPlaylistRepository(
        PlaylistSnapshot(schemaVersion: 1, playlists: [playlist('custom')]),
      );
      final service = PlaylistService(
        repository: repository,
        clock: () => _now,
  ...
  test('mutation invoked during init waits for loaded state', () async {
    final repository = DeferredLoadPlaylistRepository(
      systemSnapshot(additional: [playlist('loaded')]),
    );
    final service = PlaylistService(
      repository: repository,
      createId: () => 'created',
  ...
  test('init repair precedes a mutation accepted during load', () async {
    final repository = DeferredLoadPlaylistRepository(
      PlaylistSnapshot(schemaVersion: 1, playlists: [playlist('loaded')]),
    );
    final service = PlaylistService(
      repository: repository,
      clock: () => _now,
    ...
    expect(repository.saves, hasLength(2));
    expect(repository.saves.first.playlists.map((item) => item.id), [
      'loaded',
      'favorites',
      'recent',
    ]);
    expect(repository.saves.last.playlists.map((item) => item.id), [
      'loaded',
      'favorites',
      'recent',
      'created',
  ...
  test(
    'dispose drains pending init and its queued mutation before closing',
    () async {
  ...
  test(
    'dispose drains a pending repair init before closing revisions',
    () async {
      final repository = DeferredLoadPlaylistRepository(
        PlaylistSnapshot(schemaVersion: 1, playlists: [playlist('loaded')]),
      );
  ...
  test(
    'addToRecent updates recent without invalidating playlist pages',
    () async {
      final repository = MemoryPlaylistRepository(systemSnapshot());
      final service = PlaylistService(
        repository: repository,
  ...
  test(
    'addToRecent refreshes same-id content but exact repeats are no-ops',
    () async {
  ...
  test(
    'addToRecent globally dedupes existing ids for head and new songs',
    () async {
  ...
  test(
    'all field sorts persist reordered duplicate ids with distinct content',
    () async {
      final sortCases = <({
        ...
        (
          songs: [
            ...
            detailedSong(
              'duplicate',
            ).copyWith(duration: const Duration(seconds: 1)),
          ],
      ...
      for (final sortCase in sortCases) {
        final repository = MemoryPlaylistRepository(
          systemSnapshot(
            additional: [playlist('custom', songs: sortCase.songs)],
          ),
        );
        final service = PlaylistService(
          repository: repository,
        ...
        expect(service.getPlaylist('custom')!.songs.map(sortCase.sortedValue), [
  ...
  test(
    'reorder validates both indices before adjusted equal-index no-op',
    () async {
      final repository = MemoryPlaylistRepository(
        systemSnapshot(
          additional: [
  ...
  test(
    'successful removal, reorder, sorts, and deletion publish timestamps',
    () async {
      ...
      expect(
        await service.removeSongFromPlaylist('custom', 'medium-b'),
        isTrue,
      );
      ...
      expect(
        await service.sortSongsByArtist('custom', ascending: false),
        isTrue,
      );
  ...
  test(
    'removeSongsFromPlaylist removes multiple ids in a single revision',
    () async {
      final repository = MemoryPlaylistRepository(
        systemSnapshot(
          additional: [
      ...
      final removed = await service.removeSongsFromPlaylist('custom', [
        'a',
        'c',
        'missing',
      ]);
  ...
  test(
    'removeSongsFromPlaylist returns 0 and does not mutate when nothing matches',
    () async {
      final repository = MemoryPlaylistRepository(
        systemSnapshot(
          additional: [
  ...
  test(
    'removeSongOccurrence removes only the selected duplicate row',
    () async {
      final first = detailedSong('duplicate', name: 'First');
      final second = detailedSong('duplicate', name: 'Second');
      final repository = MemoryPlaylistRepository(
        systemSnapshot(
          additional: [
            playlist('custom', songs: [first, song('middle'), second]),
          ],
        ),
      );
      final service = PlaylistService(
      ...
      expect(await service.removeSongOccurrence('custom', 2), isTrue);
      expect(service.getPlaylist('custom')!.songs.map((item) => item.name), [
        'First',
        'Song middle',
      ]);
      expect(repository.saves, hasLength(1));
