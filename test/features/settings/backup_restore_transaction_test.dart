...
void main() {
  ...
  test('successful restore writes settings then playlists and publishes once',
      () async {
    SharedPreferences.setMockInitialValues({'theme_mode': ThemeMode.light.index});
    final storage = StorageService.forTesting(
      await SharedPreferences.getInstance(),
    ...
    expect(storage.getInt('audio_quality'), AudioQualityOption.lossless.index);
    expect(
      storage.getInt('download_quality'),
  ...
  test('decodeBackup rejects unknown keys and oversized history', () {
    ...
    expect(
      () => decodeBackup(
        '{"version":1,"playlists":$snapshot,"extra":true}',
      ),
