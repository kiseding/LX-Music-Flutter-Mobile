...
void main() {
  ...
  test(
    'decodes legacy version 1 backup playlist list through strict codec',
    () {
  ...
  test(
    'restore requires a non-null playlists field before replacement',
    () async {
  ...
  test(
    'restore strictly decodes each version 1 shape then replaces once',
    () async {
  ...
  test('rejects malformed new snapshot and unsupported outer version', () {
    ...
    expect(
      () => decodeBackupPlaylists({'version': 2, 'playlists': snapshotObject}),
      throwsFormatException,
    );
  });

  test('decodes transaction-supported playback and search settings', () {
    final decoded = decodeBackup(
      jsonEncode({
        'version': 1,
        'playlists': snapshotObject,
        'auto_resume_playback': true,
        'default_search_platform': 'wy',
      }),
    );

    expect(decoded.autoResumePlayback, isTrue);
    expect(decoded.defaultSearchPlatform, 'wy');
  });
}
