...
-- P0-1 fix: UNIQUE INDEX on (playlist_id, user_id, songmid, source). Prevents
-- duplicate rows when concurrent writes (full love-list save + incremental add)
-- both target the same song without conflict resolution.
--
-- Important: must be a NON-PARTIAL index. SQLite's ON CONFLICT clause cannot
-- target a partial index unless the INSERT also has a matching WHERE clause,
-- and our INSERT in handleLoveAdd does not — so a partial index here would
-- 500 every love-add with 'ON CONFLICT clause does not match any PRIMARY KEY
-- or UNIQUE constraint'. Empty-songmid rows (v1 leftovers) are made unique in
-- applySchemaPatches (auth.ts) before this index is created, so the full index
-- can be built without dupes.
